...
# API Key — loaded exclusively from environment variable
WEATHER_API_KEY: str = os.getenv("WEATHER_API_KEY", "")

# Provider base URL (OpenWeatherMap / WeatherAPI)
WEATHER_PROVIDER_NAME: str = "OpenWeatherMap"
WEATHER_BASE_URL: str = "https://api.openweathermap.org/data/2.5"


def get_provider_info() -> tuple[str, str]:
    """
    Determine active provider name and base URL.
    Supports WeatherAPI (weatherapi.com) and OpenWeatherMap.
    Auto-detects WeatherAPI if the key is 31 characters, or if WEATHER_PROVIDER=weatherapi.
    Defaults to OpenWeatherMap.
    """
    key = globals().get("WEATHER_API_KEY", "") or os.getenv("WEATHER_API_KEY", "")
    provider = os.getenv("WEATHER_PROVIDER", "").strip().lower()

    if provider in ("weatherapi", "weatherapi.com"):
