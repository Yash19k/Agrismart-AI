...
- ML Model predicts irrigation state: 0 (no_irrigation), 1 (irrigation_required), 2 (excess_water) + confidence
- Weather Intelligence Layer ingests current/forecast weather
- Recommendation Engine applies deterministic agronomic rules to produce actionable farmer advice
"""

from typing import Dict, Any, Optional

# Empirical confidence thresholds derived from validation calibration
...
def evaluate_weather_context(weather_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # Normalize percentage 0-100 to 0.0-1.0 if needed
    if rain_prob > 1.0:
        rain_prob = rain_prob / 100.0
    rain_prob = max(0.0, min(1.0, float(rain_prob)))

    forecast_rain_mm = float(weather_context.get("forecast_rainfall_mm", 0.0))
...
def generate_recommendation(
    ...
    Parameters:
    - ml_prediction: Dict containing 'predicted_class', 'confidence', 'status'
    - weather_context: Optional weather context (rain probability, forecast rainfall, etc.)

    Returns:
    - Dict with 'action', 'urgency', 'recommendation', 'weather_modification', and 'explanation'
    """
